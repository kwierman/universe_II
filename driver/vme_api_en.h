...
#define EN_VME_2eSSTB           0x00000200
#define EN_VME_2eSST267		0x00000400
#define EN_VME_2eSST320		0x00000800

#define EN_LINT_BIOS            0x00001000      /* LINT Universe II configuration
                                                   in BIOS in newer boards.
                                                   If BIOS gets LINT MAP wrong -- no DMA, no interrupt
                                               */


#define EN_VME_UNIVERSE_BS		( EN_VME_SCT | EN_VME_BLT /*| EN_VME_MBLT */)
...
enum ioctl_en_nums
{
	...
	IOCTL_EN_GET_VME_ENHINT_INFO,
	IOCTL_EN_SET_CAPTURE_MODE,
...
typedef struct                  /* Enhanced alternative to EN_VME_INT_INFO */
{
     UINT8 intNum;              /* VME interrupt number 1-7                  */
     UINT32 numOfInts;          /* Number of VME interrupts since last call  */
     UINT32 vecCount;           /* Number of vectors returned in the buffer  */
     UINT8 vecSize;             /* Vector size to read, 8,16 or 32           */
     UINT32 bufLen;             /* Buffer length in bytes                    */
     void *buffer;              /* Pointer to buffer for STATUSID vectors    */
...
typedef struct
{
  UINT8 ackMode;        /* Acknowledge mode */
  UINT32 numberVect;    /* Number of vectors */
...
typedef struct
{
	...
	void* usrPtr; 	 /* User passed data pointer */
}EN_VME_INT_USR_DATA;
...
int vme_readInterruptEnhInfo( INT32 deviceHandle, EN_VME_ENHINT_INFO *iPtr );
int vme_setVectorCaptureMode( INT32 deviceHandle, UINT8 iackMode, UINT32 numVectors );
...
int vme_clearSysFail( INT32 deviceHandle );
int vme_setSysFail( INT32 deviceHandle );
int vme_getSysFail( INT32 deviceHandle, UINT8 *status);
